...
import { cleanLogBody } from '../utils/logging'
...
export function logRequestResponse(
  ...
  res: Response,
  next: NextFunction,
  { ignorePaths = [], ignoreBodyKeys = [], hiddenBodyKeys = [] }: { ignorePaths: string[]; ignoreBodyKeys: string[]; hiddenBodyKeys: string[] }
) {
  // Skip some apis
  ...
  try {
    ...
    requestBody = cleanLogBody(req.body, ignoreBodyKeys, hiddenBodyKeys)
  } catch (e) {
  ...
  res.send = function (data) {
    ...
    try {
      ...
      // Ignore key specified in ignoreBodyKeys
      const body = JSON.parse(data)
      responseBody = cleanLogBody(body, ignoreBodyKeys, hiddenBodyKeys)
    } catch (e) {
